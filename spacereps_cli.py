...
def add_to_deck(deck: dict):
    ...
    while not want_to_quit:
        ...
        want_to_quit = (
            input("Press q to (q)uit, or any other key to add more cards ") == "q"
        )
...
def main():
    ...
    while True:
        ...
        while True:
            ...
            try:
                ...
                if decks_menu_selection in range(len(decks["decks"])):
                    ...
                    print(
                        "Invalid input. Please enter one of the menu options as displayed."
                    )
            except ValueError:
        ...
        while True:
            ...
            print("q: quit")
            # Get user input to select a menu option
            actions_menu_selection = input(
                f"Which action would you like to perform with {decks['decks'][decks_menu_selection]['name']}? "
            )
            try:
                # Perform the selected action to the selected deck
                decks["decks"][decks_menu_selection] = ACTIONS_MENU[
                    actions_menu_selection
                ](decks["decks"][decks_menu_selection])
            except KeyError:
